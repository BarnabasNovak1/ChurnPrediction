...
models = {
    ...
    "random_forest": load_model("rf_model.pkl"),
    ...
    "xgboost_feature_engineered": load_model("xgb-featureEngineered.pkl"),
}
...
def prepare_input(credit_score, location, gender, age, tenure, balance,
                  ...
                  estimated_salary):
    """Prepare the input dictionary and DataFrame for predictions."""
    input_dict = {
...
def make_predictions(input_df):
    """Make predictions using different models and return average probability."""
    probabilities = {
        "XGBoost": models["xgboost"].predict_proba(input_df)[0][1],
        "Random Forest": models["random_forest"].predict_proba(input_df)[0][1],
        "K-Nearest Neighbors": models["knn"].predict_proba(input_df)[0][1],
    }
    ...
    with col1:
        fig = ut_create_gauge_chart(avg_probability)
    ...
    with col2:
        fig_probs = ut.create.model_probability_chart(probabilities)
...
def generate_email(probability, input_dict, explanation, surname):
    """Generate an email based on the customer information and explanation."""
    prompt = f"""You are a manager at Novak Bank. You are responsible for ensuring customers stay with the bank and are incentivized with various offers.
